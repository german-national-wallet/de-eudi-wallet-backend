...
internal class HsmEcdsaJwsSigner(
    ...
    override fun sign(
        ...
        try {
            val signingKey = hsm.getKey(signingKeyId, HsmKeyClass.EcPrivate)
            Base64URL.encode(hsm.signEcdsaSha256(signingKey, signingInput).bytes)
...
class HsmEcdsaSigner(
    ...
    suspend fun sign(
        ...
        hsmProvider.use(spanName(jwt)) { hsm ->
            jwt.sign(HsmEcdsaJwsSigner(hsm, signingKeyId))
        }
    } catch (e: JOSEException) {
        throw JwtException.SignFailure(jwt.jwtType, e)
    }

    private fun spanName(jwt: SignedJWT) = "Sign ${jwt.jwtType} with ECDSA private key"
}
